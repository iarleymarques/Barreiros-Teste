 ...
 * Gera o arquivo PDF direto no navegador (A4 - 2 páginas) sem depender do spooler do Windows.
 */
export default function DocumentoPTImpressao({ pt, config, onVoltar }) {
  ...
  // Captura um elemento DOM como JPEG de alta definição
  const capturarImagemElemento = async (elemento) => {
    try {
      ...
      console.warn('Fallback toJpeg -> html2canvas:', err);
  ...
  // GERAÇÃO DIRETA DO PDF (2 PÁGINAS A4 COMPATÍVEL COM TODOS OS LEITORES)
  const handleBaixarPdf = async () => {
    ...
    try {
      ...
      // Captura Página 1 (Frente) e Página 2 (Verso)
      ...
      // Cria documento PDF A4 em milímetros
      const pdf = new jsPDF({
      ...
      // Página 1 (Frente - A4: 210 x 297 mm)
      pdf.addImage(imgPagina1, 'JPEG', 0, 0, 210, 297, undefined, 'FAST');

      // Página 2 (Verso)
      pdf.addPage('a4', 'portrait');
      pdf.addImage(imgPagina2, 'JPEG', 0, 0, 210, 297, undefined, 'FAST');
      ...
      alert('Houve um erro ao processar o arquivo. Tentando abrir a impressão do sistema...');
      window.print();
    } finally {
      setBaixando(false);
    }
  ...
  return (
    <div className="space-y-6">
      {/* ESTILOS DE IMPRESSÃO LIMPOS (A4, sem cabeçalhos de navegador) */}
      <style>{`
        @media print {
          body {
      ...
      <div
        ref={pagina1Ref}
        className="folha-a4 bg-white text-zinc-900 p-6 sm:p-7 shadow-2xl mx-auto border border-zinc-400 font-sans text-[11px] leading-tight w-full max-w-[820px] min-h-[1130px] flex flex-col justify-between"
      >
        <div>
      ...
      <div
        ref={pagina2Ref}
        className="folha-a4 folha-a4-quebra bg-white text-zinc-900 p-6 sm:p-7 shadow-2xl mx-auto border border-zinc-400 font-sans text-[11px] leading-tight w-full max-w-[820px] min-h-[1130px] flex flex-col justify-between"
      >
        <div>
